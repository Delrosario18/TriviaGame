class trivia {
	...
	categoryClick (event, diff) {
		...
		$('.trivia-wrap').empty().append('<h2 class="flash animated infinite">Loading</h2>');

		let timeout = setTimeout(() => {
			$('.header').append(`<h3 class="cat-title animated fadeIn">${this.props.api.results[0].category}</h3>`);
			this.question();
		}, 3000);
	...
	reset () {
		...
		$('.trivia-body').append(
			...
			'<div class="trivia-wrap animated zoomIn">' + 
				...
				'<button id="easy" class="directions diff" value="Easy">Play</button>' +
			'</div>'
...
const game = new trivia({
	...
	utils: {
		...
		numberOfAnswers: 0,
		directions: 'Select a category. Each question is worth 10,000 point. There are 10 questions and you have 60 seconds to answer each question. Good luck and hopefully you become a Millionaire!!',
	},
	api: {}
});
